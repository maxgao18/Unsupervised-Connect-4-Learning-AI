...
def random_valid(board):
    moves = [pos for pos in range(7) if board[5][pos] == 0 ]
...
def playcopy (board, player, position):
    new_board = copy.copy(board)
    for y in range(6):
        if new_board[y][position] == 0:
            new_board[y][position] = player
            break
        elif y == 6:
            print "Error: invalid move"
    return new_board
